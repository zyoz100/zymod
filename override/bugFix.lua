...
local bugFixFangXiong = GetModConfigData("bugFixFangXiong") or false

if bugFixXuaner then
    --璇儿装备bug --xe_knife
...
if bugFixFangXiong then
    if _G.isModEnableById("2456762212") then
        local old_teleport_fn = up.Get(GLOBAL.ACTIONS.TELEPORT.fn, "old_fn");
        if old_teleport_fn then
            GLOBAL.ACTIONS.TELEPORT.fn = old_teleport_fn
        end
        if GLOBAL.ACTIONS.TOUCHMEDALTOWER.fn then
            local old_touch_medal_tower_fn = up.Get(GLOBAL.ACTIONS.TOUCHMEDALTOWER.fn, "old_fn");
            if old_touch_medal_tower_fn then
                GLOBAL.ACTIONS.TOUCHMEDALTOWER.fn = old_touch_medal_tower_fn
            end
        end
    end
